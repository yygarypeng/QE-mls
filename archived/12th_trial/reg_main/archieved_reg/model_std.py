...
# %%
# Compute mean and std from the whole dataset
scaler = StandardScaler()
obs_kin_scaled = scaler.fit_transform(obs_kin)  # Preprocess Input Data

# Save computed mean and std for later inverse transformation
...
train_x = obs_kin_scaled[train_indices]
test_x = obs_kin_scaled[test_indices]
valid_x = obs_kin_scaled[valid_indices]
...
def nu_mass_loss_fn(x_batch, y_pred):
    _epsilon = 1e-10
    x_batch = x_batch * INPUT_STD + INPUT_MEAN  # Reverse transformation
...
# %%
@register_keras_serializable(package="inverse_std_scaler_layer")
class InverseStandardScalerLayer(tf.keras.layers.Layer):
    def __init__(self, mean, std, **kwargs):
        super(InverseStandardScalerLayer, self).__init__(**kwargs)
        self.mean = tf.Variable(tf.convert_to_tensor(mean, dtype=tf.float32), trainable=False)
        self.std = tf.Variable(tf.convert_to_tensor(std, dtype=tf.float32), trainable=False)

    def call(self, inputs):
        return inputs * self.std + self.mean

    def compute_output_shape(self, input_shape):
        return input_shape  # Keep the same shape

    def get_config(self):
        config = super(InverseStandardScalerLayer, self).get_config()
        config.update({"mean": self.mean.numpy(), "std": self.std.numpy()})
        return config
    ...
    def from_config(cls, config):
        ...
        # If the values are stored as dictionaries (from __numpy__), convert them back
        if isinstance(mean, dict) and "config" in mean:
            mean = np.array(mean["config"]["value"], dtype=np.float32)
        if isinstance(std, dict) and "config" in std:
            std = np.array(std["config"]["value"], dtype=np.float32)
...
def build_model(input_shape):
    ...
    lep0 = InverseStandardScalerLayer(mean=INPUT_MEAN[..., :4], std=INPUT_STD[..., :4])(lep0)
    lep1 = InverseStandardScalerLayer(mean=INPUT_MEAN[..., 4:8], std=INPUT_STD[..., 4:8])(lep1)
    ...
    for _ in range(2):
        x = residual_block(x, 512, dropout_rate=0.4, l2=1e-3)
        x = residual_block(x, 256, dropout_rate=0.4, l2=1e-3)
    for _ in range(2):
        x = residual_block(x, 128, dropout_rate=0.4, l2=1e-3)
    ...
    # Bottleneck
    x = tf.keras.layers.Dense(8, activation=None, kernel_initializer="he_normal")(x)
    x = tf.keras.layers.BatchNormalization()(x)
...
model = tf.keras.models.load_model(
    ...
    custom_objects={
        ...
        "InverseStandardScalerLayer": InverseStandardScalerLayer,
    }
)
# Manually recompile the setup (need to remember these parameters!)
model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
    loss_weights={
        "mae": 1.0,
...
nu0_4vect = sig_pred_inv[:, :4] - (test_x[:, :4] * INPUT_STD[..., :4] + INPUT_MEAN[..., :4])
nu1_4vect = sig_pred_inv[:, 4:8] - (test_x[:, 4:8] * INPUT_STD[..., 4:8] + INPUT_MEAN[..., 4:8])
nu0_mass_squared = (
